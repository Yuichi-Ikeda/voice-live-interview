...
class VoiceSessionHandler:
    ...
    async def _setup_session(self, connection):
        ...
        recognition_language = config.get("recognitionLanguage", "ja-JP")
        is_realtime = model and "realtime" in model
        input_audio_transcription = AudioInputTranscriptionOptions(
            model="whisper-1" if (mode == "model" and is_realtime) else sr_model,
            language=None if (sr_model == "mai-ears-1" or recognition_language == "ja-JP")
    ...
    def _build_voice_config(self, config: dict):
        ...
        voice_name = config.get("voiceName", os.getenv("VOICELIVE_VOICE", "ja-jp-nanami:DragonHDLatestNeural"))
        voice_temperature = config.get("voiceTemperature", 0.8)
