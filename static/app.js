...
function updateConditionalFields() {
    ...
    // Cascaded models
    const cascadedModels = ['gpt-4.5', 'gpt-4.1', 'gpt-4.1-mini', 'gpt-4.1-nano', 'gpt-4o', 'gpt-4o-mini', 'phi4-mm', 'phi4-mini'];
...
function handleServerMessage(msg) {
    ...
    switch (type) {
        ...
        case 'transcript_delta':
            // Skip streaming transcript for assistant - only show final in transcript_done
            break;
        case 'text_delta':
            // Skip text deltas in chat — only show actual spoken transcript (transcript_delta)
            break;
        case 'text_done':
            // Text response complete — chat uses transcript_done instead
            break;
        case 'speech_started':
        ...
        case 'response_created':
            pendingAssistantText = '';
            // Don't create message bubble yet - wait for transcript_done
            isSpeaking = true;
            break;
        case 'response_done':
            isSpeaking = false;
